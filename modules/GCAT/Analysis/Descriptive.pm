...
require Exporter;
our @ISA = qw(Exporter);
our @EXPORT_OK = qw(get_Descriptive_Statistics get_Character_Statistics);
...
sub get_Character_Statistics {
	...
	# setup variables
	my @data = ();
	my ($gene_count, $genome_size, $transcriptome_size, $exome_size, $intronome_size, $transcript_count, $exon_count, $intron_count, 
		$fputr_size, $tputr_size, $introns_per_gene, $introns_per_bond, $exons_per_gene, $exons_per_gl, $repeat_size, $repeat_count, $repeat_density) = 0;
	my @colnames = ("Total Gene Count",
					"Total Genome Size",
					"Transcriptome Size",
					"Exome Size",
					"Intronome Size",
	...
	foreach my $org (@rownames) {
		...
		($intronome_size, $intron_count) = &get_Intronome_Size($registry, $org);
		...
		$introns_per_bond = ($intron_count / ($transcriptome_size - $transcript_count));

		# get exons per gene
		print "Getting exon densities (per gene/% gene length)...\n";
		$exons_per_gene = ($exon_count / $transcript_count);
		$exons_per_gl = $exon_count / (($transcriptome_size + $intronome_size) / 100);

		# get repeat element size
		print "Getting repeat element statistics (size/count/per gene density)...\n";
		($repeat_size, $repeat_count, $repeat_density) = &get_Repeat_Stats($registry, $org);

		push(@data, ($gene_count, $genome_size, $transcriptome_size, $exome_size, $intronome_size, $transcript_count, $exon_count, $intron_count, 
						$fputr_size, $tputr_size, $introns_per_gene, $introns_per_bond, $exons_per_gene, $exons_per_gl, $repeat_size, $repeat_count, $repeat_density));
	}
...
sub get_Descriptive_Statistics {
	...
	# let user know where we have outputted
