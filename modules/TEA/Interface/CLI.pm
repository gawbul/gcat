...
=head1 DESCRIPTION
	...
	A module that provides a command line text based input for TEA.

=cut

package TEA::Interface::CLI;
...
sub setup_CLI_ENV {
	...
	my $dir = getcwd();
	my $path = File::Spec->catfile($dir, 'modules', 'TEA', 'Scripts');
...
sub banner {
    ...
    print "Toolkit for Evolutionary Analysis\n";
	print "Command Line Interface version, $ver\n";
	print "Copyright (C) 2010 Steve Moss\n";
	print "Platform: $os_name $os_arch ($os_ver)\n\n";
	print "TEA is free software and comes with ABSOLUTELY NO WARRANTY.\n";
	...
	print "Type 'exit' or 'quit' to quit TEA.\n";
}
...
sub cmd_parser {
	...
	# check if in scripts
		...
		my $dir = getcwd();
		my $path = File::Spec->catfile($dir, 'modules', 'TEA', 'Scripts', $cmd . ".pl");
